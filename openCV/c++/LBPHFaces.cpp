...
void detect_faces(cv::Mat& img, Ptr<LBPHFaceRecognizer>&model)
{
  ...
  Mat testFrame = img;
  cv::cvtColor(testFrame, testFrame, COLOR_BGR2GRAY);
  ...
  for (auto& face : faces) {
    ...
    putText(img, names[predictedLabel], cv::Point(face.tl().x , face.tl().y - 40), FONT_ITALIC, 1 , cv::Scalar(255, 0, 255), 2);
  }
}
